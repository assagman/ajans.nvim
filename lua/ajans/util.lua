...
function M.notify(msg, level)
  ...
  vim.schedule(function()
    vim.notify(msg, level or vim.log.levels.INFO, { title = "Ajans" })
  end)
end
...
function M.debug(msg, what)
  if require("ajans.config").debug then
    if what ~= nil then
...
function M.set_extmark(buffer, ns_id, row, col, opts)
  ...
  if not ok then
    local e = vim.deepcopy(opts) --[[@as ajans.Extmark]]
    e.row, e.col = row, col
