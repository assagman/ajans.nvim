...
M.terminals = {} ---@type table<string, ajans.cli.Terminal>
...
---@type vim.wo
local wo = {
  winhighlight = "Normal:AjansChat,NormalNC:AjansChat,EndOfBuffer:AjansChat,SignColumn:AjansChat",
  colorcolumn = "",
  cursorcolumn = false,
  cursorline = false,
  fillchars = "eob: ",
...
function M:init()
  ...
  self.send_queue = {}
  self.group = vim.api.nvim_create_augroup("ajans_cli_" .. self.id, { clear = true })
  M.terminals[self.id] = self
  if Config.cli.win.config then
    Config.cli.win.config(self)
  end
  self.scrollback = require("ajans.cli.scrollback").new(self)
  return self
end
...
function M:start()
  ...
  self:bo()
  vim.b[self.buf].ajans_cli = self.tool
  ...
  -- restore mode when entering the ajans window
  ...
  if Config.cli.watch then
    require("ajans.cli.watch").enable()
  end
end
...
function M:open_win()
  ...
  vim.w[self.win].ajans_cli = self.tool
  vim.w[self.win].ajans_session_id = self.id
  self:wo()
end
...
function M:close()
  ...
  if vim.tbl_isempty(M.terminals) then
    require("ajans.cli.watch").disable()
  end
  Session.detach(self)
  if self.timer and not self.timer:is_closing() then
    self.timer:close()
...
function M:keys(buf)
  buf = buf or self.buf
  local Actions = require("ajans.cli.actions")
  ...
  for name, km in pairs(keys) do
    if type(km) == "table" then
      ...
      elseif not action then
        ...
        km_opts.buffer = buf
        km_opts.desc = km_opts.desc or ("Ajans: %s"):format(name:gsub("^%l", string.upper))
