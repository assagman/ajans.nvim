...
---@class ajans.context.loc.Opts
---@field kind? "file"|"line"|"position"

---@param ctx ajans.context.Loc|ajans.context.ctx
---@param opts? ajans.context.loc.Opts
---@return ajans.Text[]
  ...
  local function add(...)
    for _, v in ipairs({ ... }) do
      if type(v) == "number" then
        ret[#ret + 1] = { tostring(v), "AjansLocNum" }
      elseif v == "L" then
        ret[#ret + 1] = { v, "AjansLocRow" }
      elseif v == "C" then
        ret[#ret + 1] = { v, "AjansLocCol" }
      else
        ret[#ret + 1] = { v, "AjansLocDelim" }
      end
    end
  end

  add("@")
  ret[#ret + 1] = { name, "AjansLocFile" }
