...
local Diag = require("ajans.cli.context.diagnostics")
...
M.context = {
  ...
  selection = function(ctx)
    return require("ajans.cli.context.selection").get(ctx)
  end,
...
---@class ajans.context.ctx
---@field win integer
---@field buf integer
---@field cwd string
---@field row integer (1-based)
---@field col integer (1-based)
---@field range? ajans.context.Range

---@alias ajans.context.Fn.ret string|string[]|ajans.Text|ajans.Text[]|false
---@alias ajans.context.Fn fun(ctx: ajans.context.ctx): ajans.context.Fn.ret?

---@class ajans.context.Range
---@field from ajans.Pos (1,0)-based
---@field to ajans.Pos (1,0)-based
---@field kind "line"|"char"|"block"

---@class ajans.Context
---@field ctx ajans.context.ctx
---@field context table<string, ajans.Text[]|false>
local C = {}
C.__index = C
...
---@param opts string|ajans.cli.Message|{this?: boolean}
function C:render(opts)
  opts = type(opts) == "string" and { msg = opts } or opts --[[@as ajans.cli.Message|{this?:boolean}]]
  opts.msg = opts.msg or ""

  local lines = opts.msg == "" and {} or vim.split(opts.msg, "\n", { plain = true })

  if opts.prompt then
    ---@type ajans.Prompt|string|fun(ctx:ajans.context.ctx):(string?)|nil
    ...
    ---@cast prompt ajans.Prompt|string
...
function M.ctx()
  ...
  local wins = vim.tbl_filter(function(w)
    ...
    return vim.bo[buf].filetype ~= "ajans_terminal"
  end, vim.api.nvim_list_wins())
  table.sort(wins, function(a, b)
    return (vim.w[a].ajans_visit or 0) > (vim.w[b].ajans_visit or 0)
  end)
  local win = wins[1] or vim.api.nvim_get_current_win()
  local buf = vim.api.nvim_win_get_buf(win)
  local cursor = vim.api.nvim_win_get_cursor(win)
  ---@type ajans.context.ctx
  return {
    win = win,
    buf = buf,
