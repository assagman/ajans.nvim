...
---@alias ajans.DiffText {text: string, lines: string[], virt_lines: ajans.Text[]}

---@class ajans.Diff
---@field range {from: ajans.Pos, to: ajans.Pos}
---@field hunks ajans.diff.Hunk[]
---@field from ajans.DiffText
---@field to ajans.DiffText

---@class ajans.diff.Hunk
---@field pos ajans.Pos
---@field cover integer the number of lines covered in the "from" text
---@field kind "add" | "delete" | "change"
---@field inline? boolean
---@field extmarks ajans.Extmark[]

--- Calculate the result covering full lines
---@param edit ajans.NesEdit
...
---@param diff ajans.Diff
function M.diff_lines(diff)
  local hunks = M._diff(diff.from.lines, diff.to.lines, DIFF_OPTS)
  local dels = {} ---@type table<integer, {hunk: ajans.diff.Hunk}>
  local adds = {} ---@type table<integer, {hunk: ajans.diff.Hunk, virt_lines: ajans.Text[]}>

  local width = 0
  for _, hunk in ipairs(hunks) do
    local ai, ac, bi, bc = unpack(hunk)

    local inline_hunks = {} ---@type ajans.diff.Hunk[]
  ...
  for row, info in pairs(dels) do
    table.insert(info.hunk.extmarks, {
      ...
      virt_text = { { string.rep(" ", vim.o.columns), "AjansDiffContext" } },
      line_hl_group = "AjansDiffDelete",
    })
  end
...
function M.diff_inline(diff, from_idx, to_idx)
  ...
  local ret = {} ---@type ajans.diff.Hunk[]
