...
---@param res {edits: ajans.lsp.NesEdit[]}
---@type lsp.Handler
function M._handler(err, res, ctx)
  ...
  for _, edit in ipairs(res.edits or {}) do
    local e = require("ajans.nes.edit").new(client, edit)
...
function M.apply()
  ...
  vim.schedule(function()
    ...
    vim.schedule(function()
      ...
      Util.emit("AjansNesDone", { client_id = client.id, buffer = buf })
    end)
